...
public abstract class AbstractDbTable<K, V> implements IDbTable<K,V> {
    ...
    @Override
    public List<V> select(Predicate<V> check, Comparator<V> compare) {
        return valueMap.values().stream().filter(check).sorted(compare).collect(Collectors.toUnmodifiableList());
    }

    @Override
    public List<V> getAll(Comparator<V> compare) {
        return valueMap.values().stream().sorted(compare).collect(Collectors.toUnmodifiableList());
    }
}
